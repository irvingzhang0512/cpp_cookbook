...
CH264Decoder::CH264Decoder() { initial(); }

CH264Decoder::~CH264Decoder() { unInitial(); }
...
int CH264Decoder::decode(uint8_t *pDataIn, int nInSize, cv::Mat &res) {
  ...
  if (packet.size > 0) {
    ...
    if (ret == 0)
      got_picture = avcodec_receive_frame(
          context, frame);  // got_picture = 0 success, a frame was returned
    if (ret < 0) {
