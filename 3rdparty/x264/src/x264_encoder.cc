...
x264Encoder::x264Encoder(int videoWidth, int videoHeight, int channel,
                         int fps) {
  Init();
...
bool x264Encoder::Create(int videoWidth, int videoHeight, int channel,
                         int fps) {
  int ret;
  int imgSize;
  ...
  ret = x264_param_default_preset(&m_encoder->param, m_encoder->preset,
                                  m_encoder->tune);
  if (ret < 0) {
  ...
  /*初始化pic*/
  ret = x264_picture_alloc(&m_encoder->pic_in, m_encoder->colorspace, m_width,
                           m_height);
  if (ret < 0) {
...
int x264Encoder::EncodeOneFrame(const cv::Mat& frame) {
  ...
  memcpy(m_encoder->pic_in.img.plane[2], yuv.data + m_lumaSize + m_chromaSize,
         m_chromaSize);
  m_encoder->pic_in.i_pts = m_encoder->iframe++;

  m_encoder->iframe_size =
      x264_encoder_encode(m_encoder->h, &m_encoder->nal, &m_encoder->inal,
                          &m_encoder->pic_in, &m_encoder->pic_out);
